...
namespace WebApiUWPDemo
{
    ...
    /// </summary>
    public sealed partial class SendInformationContent : Page
    {
        string imageName = "";
        ...
        const string uriPicturePrefix = "https://myblobsample.blob.core.windows.net/mycontainer/";

        const string informationId = "1";

        public object InformationDetails { get; private set; }

        public SendInformationContent()
        {
        ...
        private async void btnSendContent_Click(object sender, RoutedEventArgs e)
        {
            ...
            if(txtContent.Text.Length>0)
            {
                ...
                {
                    string strImageName = Guid.NewGuid().ToString("N")+".jpg";
                    string uriImage = uriPicturePrefix + strImageName;
                    uriImages.Add(uriImage);
                    await blob.uploadImage(strImageName, App.list[i]);
                }
                string xmlContent = GenerateXMLString(txtContent.Text, App.list);
                if(xmlContent!=null)
                {
                    txtMessage.Text = "生成的xml为：" + xmlContent;
                    InformationDetails information =new 
                }
        ...
        private string GenerateXMLString(string strContent,List<StorageFile> imgFiles)
        {
            try
            {
                XmlDocument xmlDoc = new XmlDocument();
                XmlElement rootElement = xmlDoc.CreateElement("string");
                xmlDoc.AppendChild(rootElement);

                XmlElement contentElement = xmlDoc.CreateElement("p");
                contentElement.InnerText = strContent;
                rootElement.AppendChild(contentElement);

                XmlElement imgElement_p = xmlDoc.CreateElement("p");
                int imgNumber = imgFiles.Count;
                for (int i = 0; i < imgNumber; i++)
                {
                    XmlElement imgElement = xmlDoc.CreateElement("img");
                    imgElement.SetAttribute("src", uriImages[i]);
            ...
            {
                txtMessage.Text = "xml生成异常，异常原因：" + ex.Message;
                return null;
            }
        }
